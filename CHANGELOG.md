...
- **Password-protected documents (all encryptable formats).** Encrypted OOXML (`.docx`/`.xlsx`/`.pptx`; ECMA-376 both *agile* AES-CBC and *standard* AES-ECB, unwrapped from their CFB/OLE container) and encrypted ODF (`.odt`/`.ods`/`.odp`/`.odg`; per-entry AES-256/192/128-CBC with PBKDF2) now decrypt and parse, joining the PDF password support. One unified, top-level config drives them all: **`password`** and the lazy/interactive **`onPassword`** callback (moved here from `pdfParserConfig` so a single option covers every format), with unified error codes **`PASSWORD_REQUIRED`** (encrypted, none supplied) and **`PASSWORD_INCORRECT`** (wrong). Decryption happens before type detection, in a new zero-dependency `crypto` module (Node's `crypto` plus native WebCrypto PBKDF2, polyfilled in the prebuilt browser bundle) with a minimal CFB reader; the unsupported schemes raise a clear error rather than a wrong-password one: OOXML's legacy RC4/CryptoAPI and ODF's legacy Blowfish, and ODF's newer AES-256-GCM with Argon2id key derivation (the default since LibreOffice 24.8) — re-save with the classic AES-CBC/PBKDF2 scheme to parse it. Untrusted input is bounded document-wide, not just per entry: the second-layer decompression and the per-entry PBKDF2 work both accumulate against the configured budget so a many-entry file cannot multiply past it. Fixtures are generated independently of the decryptor by `scripts/generate-crypto-fixtures.mjs`.
