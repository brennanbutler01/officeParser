...
- **`NodeBounds`** type and error/warning codes `PASSWORD_REQUIRED`, `PASSWORD_INCORRECT`, `PDF_STRUCT_TREE_UNRELIABLE`, and **`PDF_NO_TEXT_EXTRACTED`** (a warning that a PDF yielded essentially no text, so it is likely scanned; enable `ocr` with `extractAttachments`).
