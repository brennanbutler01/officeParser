...
- **`ocr: true` without `extractAttachments: true` no longer silently does nothing (and no longer decodes every page image for nothing).** Page-image OCR runs over collected images, which need `extractAttachments`, so the combination produced no text; it now raises the new `OCR_REQUIRES_ATTACHMENTS` warning and skips the wasted image decode. Set both flags to OCR a scanned PDF.
