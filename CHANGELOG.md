...
- **PDF table `colSpan` recovery.** A tagged PDF encodes a horizontally merged cell as a wide cell plus empty placeholder cells filling the columns it covers (pdf.js does not expose the `/ColSpan` attribute). Those placeholders are now absorbed: a non-empty cell whose box extends past an adjacent empty column takes it over and reports `colSpan`, so the generated table shows the merge instead of stray empty cells. Conservative by construction (only empty placeholders a cell's own geometry provably covers are merged), so regular grids are untouched. Needs page geometry, so it is off under `ignorePositions`.
